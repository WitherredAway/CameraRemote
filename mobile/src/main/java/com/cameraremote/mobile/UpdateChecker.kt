...
class UpdateChecker(private val context: Context) {
    ...
    companion object {
        private const val TAG = "CameraRemoteUpdate"
        private const val GITHUB_API_URL = "https://api.github.com/repos/WitherredAway/CameraRemote/releases/latest"
    ...
    fun downloadAndInstall(downloadUrl: String) {
        try {
            val fileName = "CameraRemote-update.apk"
            val file = File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), fileName)
            if (file.exists()) file.delete()

            val request = DownloadManager.Request(Uri.parse(downloadUrl))
                .setTitle("Camera Remote Update")
    ...
    fun downloadWatchApk(watchDownloadUrl: String, callback: (File?) -> Unit) {
        try {
            val fileName = "CameraRemote-Watch-update.apk"
            val file = File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), fileName)
            if (file.exists()) file.delete()

            val request = DownloadManager.Request(Uri.parse(watchDownloadUrl))
                .setTitle("Camera Remote Watch Update")
